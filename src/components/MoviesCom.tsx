...
export default function MoviesCom() {
    ...
    const listPoster: Array<MovieTrend> = listMovie.listMoviesTrend.slice(3, 10)
    const listPosterPop: Array<MovieTrend> = listMovie.listMoviesPopular.slice(5, 20)
    return (
        <Container>
        <Header style={{
            ...
            <Body>
                <Title style={{ color: 'black', fontSize: 25, paddingHorizontal: 50 }}>MOVIE</Title>
            </Body>
            <Right >
            ...
            <List horizontal={true} dataArray={listPoster} // your array should go here
                    renderRow={pet => (
                        <ListItem style={{ borderBottomWidth: 0, flexDirection: 'column' }}>
                            ...
                            <Text style={{ width: 150, textAlign: 'center', height: 50, color:'black' }}>{pet.original_title}</Text>
                        </ListItem>
                    )}>
                </List>
            <Text style={{ paddingLeft: 20 }}>POPULAR</Text>
            <List horizontal={true} dataArray={listMovie.listMoviesPopular.slice(0,7)} // your array should go here
                    renderRow={pet => (
                        <ListItem style={{ borderBottomWidth: 0, flexDirection: 'column' }}>
                            <Image style={{ width: 150, height: 200, borderRadius: 20 }} source={{ uri: 'https://www.themoviedb.org/t/p/w600_and_h900_bestv2/' + pet.poster_path }}></Image>
                            <Text style={{ width: 150, textAlign: 'center', height: 50, color:'black' }}>{pet.original_title}</Text>
